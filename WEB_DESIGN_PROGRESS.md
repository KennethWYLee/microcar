...
## 2026-09-20 修正主題 06 與 09 的教材說明

- 主題 06 的 `controler_ble.7z` 用途改為「Pico 端 Python 程式壓縮檔」，
  與既有下載內容及小車端程式說明一致。修改產生器、確認預計差異後，
  僅重建主題 06 頁面；教材下載副本沒有異動。
- 主題 09 移除「83 張延伸版暫不公開」的舊說明，明確寫出本頁可下載
  83 張課堂強化版，保留 75 張延伸版暫不公開的既有範圍。
- 修改前後 `tools/build_case_topic_pages.py --check` 均通過；修正後
  `tools/verify_site.py` 通過，涵蓋 19 個 HTML、649 個本機引用與錨點、
  26 個 Python，以及既有程式、下載檔與素材檢查。
- 本機瀏覽器已檢查兩頁在 390×667 與 1280×900 的文字及版面，無頁面橫向
  溢出；主題 06 檔案樹維持區塊內橫向捲動，主題 09 說明可正常換行。
- 上述檢查完成時尚未 commit、push 或發布；使用者隨後要求 commit 與 push。
  未測試實體硬體。

## 2026-09-20 調整導覽、字型與平板排版

- 依使用者核准的四項建議，修正未定義的字型與顏色變數，沿用既有等寬字型、
  深藍及正文顏色。保留網站原有配色、卡片風格與教材內容。
- 820px 以下保留「開始設定」捷徑，其餘導覽可由「主題選單」展開。
  導覽文字改為 14px，按鈕與連結高度至少 44px；選單支援鍵盤、Escape 收合、
  展開狀態與控制目標標示，切換尺寸時維持可操作的焦點。
  導覽僅在 JavaScript 初始化完成後收合，未執行 JavaScript 時仍顯示原有連結。
- 放寬主標題可用寬度、增加行距；內頁標題採 32–46px、1.25 倍行距。
  術語段落跳轉預留桌面導覽高度，手機使用 24px 間距。
- 主題卡、下載卡與 Case 目錄在 561–1100px 使用兩欄，560px 以下單欄。
- 19 個 HTML 僅更新 CSS 與 JavaScript 資源版本為 `20260920-layout`；
  01–08 頁面由產生器重建，下載教材沒有異動。
- `tools/build_case_topic_pages.py --check`、`tools/verify_site.py` 通過：
  19 個 HTML、649 個本機引用與錨點、26 個 Python 及既有程式與下載素材檢查。
- 瀏覽器驗證：19 頁在 390px 寬度皆無頁面橫向溢出，選單初始化正常；
  首頁、主題 01、下載區另測 320、560、561、820、821、1024、1280px 寬度，
  欄數符合設定且無頁面橫向溢出。390×667 首頁的提示與導覽底部由約 410px
  降至 283px，開始設定捷徑及首頁主要按鈕均在首屏內。
- 已確認手機選單展開、Tab、Enter、Escape、頁面導向與雙向尺寸切換焦點；
  主題 09 桌面標題及術語跳轉可讀，主題 01 行內與區塊程式均為等寬字型，
  BootCamp 程式載入及複製成功。檢查期間未記錄 JavaScript 錯誤。
- 使用者已授權修正後 commit 與 push；部署結果於推送後另行確認。
  本次驗證為網站靜態與瀏覽器檢查，未測試實體硬體。
